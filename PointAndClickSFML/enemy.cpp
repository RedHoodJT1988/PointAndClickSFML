...
bool Enemy::performSetup()
{
	...
	enemySprite.setPosition(sf::Vector2f(255, 400));
...
{
	flaot enemyMinX = enemySprite.getGlobalBounds().left;
	float enemyMaxX = enemySprite.getGlobalBounds().width + enemyMinX;
	float enemyMinY = enemySprite.getGlobalBound().top;
	float enemyMaxY = enemySprite.getGlobalBounds().height + enemyMinY;

	float mouseX = mousePos.x;
	float mouseY = mousePos.y;

	return moouseX >= enemyMinX && mouseX <= enemyMaxX && mouseY >= enemyMinY && mouseY <= enemyMaxY;
}
